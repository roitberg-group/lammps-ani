...
def analyze_all_frames(
    ...
    save_interval = 100  # Interval for saving dataframes
    ...
    time_offset += local_start_frame * timestep * dump_interval * 1e-6
    total_frames_in_segment = end_frame - local_start_frame
    print(f"Total frames: {total_frames}, total frames in segment: {total_frames_in_segment}")
    print("time offset is", time_offset, ", segment length is", segment_length)
    ...
    frame_num = 0
    output_filename = f"{Path(traj_file).stem}_seg{segment_index}of{num_segments}"
