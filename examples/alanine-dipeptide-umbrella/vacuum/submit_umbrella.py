...
for i, phi in enumerate(phi_values):
    for j, psi in enumerate(psi_values):
        ...
        command = f"python run_one.py alanine-dipeptide.vacuum.data --kokkos --num_gpus=1 --input_file=in.vacuum.lammps --log_dir={log_dir} --ani_model_file='ani2x_b973c.pt' --run_name=vacuum_umbrella_ani2x --ani_num_models=1 --timestep=0.5 --label={label} --run"
