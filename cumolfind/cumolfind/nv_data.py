...
class Data:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if edge_index is not None and edge_index.dtype not in [
            torch.int32,
            torch.int64,
        ]:
            raise ValueError(
                f"Argument `edge_index` needs to be of type `torch.int32` or `torch.int64` but "
                f"found type `{edge_index.dtype}`."
            )

        if face is not None and face.dtype not in [torch.int32, torch.int64]:
            raise ValueError(
                f"Argument `face` needs to be of type `torch.int32` or `torch.int64` but "
                f"type `{face.dtype}`."
            )
    ...
    def apply(self, func: Callable[[Any], Any], *keys: str) -> "Data":
    ...
    def to(self, device: torch.device, *keys: str, **kwargs: Any) -> "Data":
    ...
    def cpu(self, *keys: str) -> "Data":
        r"""Copies all attributes :obj:`*keys` to CPU memory.
        If :obj:`*keys` is not given, the conversion is applied to all present
        attributes."""
        return self.apply(lambda x: x.cpu(), *keys)

    def cuda(self, device=None, non_blocking=False, *keys: str) -> "Data":
    ...
    def pin_memory(self, *keys: str) -> "Data":
        ...
        if self.edge_index is not None:
            if self.edge_index.dtype not in [torch.int32, torch.int64]:
                raise RuntimeError(
                    ...
                    ).format(torch.int32, self.edge_index.dtype)
                )
        ...
        if self.face is not None:
            if self.face.dtype not in [torch.int32, torch.int64]:
                raise RuntimeError(
                    ...
                    ).format(torch.int32, self.face.dtype)
                )
