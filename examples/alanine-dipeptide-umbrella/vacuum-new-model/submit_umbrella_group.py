...
for i, phi in enumerate(phi_values):
    for j, psi in enumerate(psi_values):
        ...
        with open(f'{log_dir}/{label}.plumed.dat', 'w') as file:
            file.write(f'MOLINFO STRUCTURE=alanine-dipeptide.vacuum.pdb\n')
        ...
        command = f"python run_one.py alanine-dipeptide.vacuum.data --kokkos --num_gpus=1 --input_file=in.lammps --log_dir={log_dir} --ani_model_file='ani2x_solvated_alanine_dipeptide.pt' --run_name=umbrella --ani_num_models=1 --timestep=0.5 --label={label} --run\n"
